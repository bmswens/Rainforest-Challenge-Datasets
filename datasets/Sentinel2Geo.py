...
class Sentinel2Geo(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # paths
        self.root = root
        self.tgt = os.path.join(self.root, self.dataset_name, self.label)
        self.tgt_url = self.train_url if train else self.test_url

        # make root if not exists
    ...
    def _download(self):
        ...
        zip_path = os.path.join(self.root, f"{self.dataset_name}-{self.label}.zip")
        ...
        with ZipFile(zip_path, 'r') as zip_file:
            zip_root = zip_file.namelist()[0].replace('/', '')
            zip_file.extractall(self.root)

        # move extracted folder to target
        extracted_folder = os.path.join(self.root, zip_root)
        if os.path.exists(self.tgt):
